    ...
    function getStoreNextOpeningTime(data) {
        ...
        if (todayData && todayData.timeslot.length) {
            ...
            if (futureToday.length) {
                return `Next available today at ${getTimeFormat(futureToday[0].from)}`;
            }
        }
        ...
        for (let i = 1; i <= 7; i++) {
            ...
            if (dayData && dayData.timeslot.length) {
                ...
                if (i === 1) {
                    return `Next available tomorrow at ${getTimeFormat(nextSlot.from)}`;
                } else {
                    return `Next available ${nextDay} at ${getTimeFormat(nextSlot.from)}`;
                }
            }
        }
        return "Closed";
